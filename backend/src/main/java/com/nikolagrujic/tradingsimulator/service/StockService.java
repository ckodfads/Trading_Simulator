...
public class StockService {
    ...
    public StockInfo getRecommendedStock() {
        ...
        for (Transaction transaction : transactions) {
            if (values.containsKey(transaction.getSymbol())) {
                values.put(transaction.getSymbol(),
                        values.get(transaction.getSymbol()).add(transaction.getTradePrice()));
            } else {
    ...
    public void startRetrievingStocks() {
        for (StockExchange stockExchange : StockExchange.values()) {
            try {
                ...
                ResponseEntity<StocksListResponse> responseEntity = restTemplate.exchange(requestEntity,
                        StocksListResponse.class);
    ...
    private BigDecimal fetchPrice(String symbol) throws Exception {
        try {
            ...
            ResponseEntity<PriceResponse> responseEntity = restTemplate.exchange(requestEntity, PriceResponse.class);
    ...
    @Async
    @Scheduled(initialDelay = Constants.REQUEST_DELAY_MILLISECONDS, fixedDelay = PULL_STOCK_PRICES_PERIOD_MILLISECONDS)
